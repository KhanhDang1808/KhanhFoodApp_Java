...
import com.example.food.Presenter.SetOnItemClick;
import com.example.food.R;

import java.text.NumberFormat;
import java.util.ArrayList;
//com.example.appfoodv2
public class GioHangAdapter extends RecyclerView.Adapter<GioHangAdapter.ViewHodler> {
    ...
    public class ViewHodler extends RecyclerView.ViewHolder implements  View.OnClickListener {
        ...
        public ViewHodler(@NonNull View itemView) {
            ...
            itemView.findViewById(R.id.minus_btn).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    int number =Integer.parseInt(txtsoluong.getText().toString());
                    if(number >1) {
                        number-=1;
                        txtsoluong.setText(number+"");
                    }
                }
            });
            itemView.findViewById(R.id.plus_btn).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    int number = Integer.parseInt(txtsoluong.getText().toString());
                    number+=1;
